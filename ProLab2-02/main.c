...
char *decode(struct token *tokens, int numTokens, int *pcbDecoded){
    ...
    *pcbDecoded = 0;
    ...
    for (i = 0; i < numTokens; i++){
        ...
        int l = GETLENGTH(tokens[i].offset_len);
        char c = tokens[i].c;

        if (*pcbDecoded + l + 1 > cap){
            cap = cap << 1;
            decoded = realloc(decoded, cap);
        }

        if (l > 0){
            lz77memcpy(&decoded[*pcbDecoded], &decoded[*pcbDecoded - offset], l);
        }
...
struct token *encode(char *text, int limit, int *numTokens){
    ...
    for (lookahead = text; lookahead < text + limit; lookahead++){
        ...
        for (; search < lookahead; search++){
            int l = prefix_match_length(search, lookahead, LENGTHMASK);
...
char *file_read(FILE *f1, int *size){
    char *content;
    fseek(f1, 0, SEEK_END);
    *size = ftell(f1);
    content = malloc(*size);
    fseek(f1, 0, SEEK_SET);
    fread(content, 1, *size, f1);
    return content;
}
...
int main(void){
    ...
    char *kaynak = "aaaaaaaa", *decoded_metin = "";
    struct token *encoded_metin;

    if (f1 = fopen("oku.txt", "rb")){
        kaynak = file_read(f1, &metin_boyutu);
        fclose(f1);
    }

    encoded_metin = encode(kaynak, metin_boyutu, &token_sayisi);

    if (f1 = fopen("encoded.txt", "wb")){
        fwrite(encoded_metin, sizeof(struct token), token_sayisi, f1);
        fclose(f1);
    }

    decoded_metin = decode(encoded_metin, token_sayisi, &decode_boyutu);

    if (f1= fopen("decoded.txt", "wb")){
        fwrite(decoded_metin, 1, decode_boyutu, f1);
        fclose(f1);
    }

    printf("Original Size: %d, Encode Size: %d, Decode Size: %d", metin_boyutu, token_sayisi * sizeof(struct token), decode_boyutu);
